...
class ServiceFactory implements AbstractFactoryInterface
{
    ...
    {
        return $serviceLocator->has('repositorymanager') && (strpos($name, 'repository:') === 0);
    }
    ...
    {
        $name = substr($name, strlen('repository:'));
        return $serviceLocator->get('repositorymanager')->get($name);
    }
}