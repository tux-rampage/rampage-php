...
class UserConfig extends xml\AggregatedXmlConfig implements UserConfigInterface, url\UrlConfigInterface
{
    ...
    protected function getDefaultMergeRulechain()
    {
        ...
        $rules->add(new userconfig\PropertyMergeRule('~/property$~'))
              ->add(new xml\mergerule\AllowSiblingsRule('~/item$~'));
    ...
    public function getConfigValue($name, $default = null, $domain = null)
    {
        ...
        if ($domain == '__default__') {
            ...
            if (!$node instanceof xml\SimpleXmlElement) {
                $node = $this->getNode("./property[@name = $xpathName and not(@domain)]");
            }
        }
    ...
    protected function processConfigVariables($value, $prefix = null)
    {
        if ($prefix && !preg_match('~^[a-z0-9._-]*$~i', $prefix)) {
            throw new exception\InvalidArgumentException('Invalid config var prefix: ' . $prefix);
        }
