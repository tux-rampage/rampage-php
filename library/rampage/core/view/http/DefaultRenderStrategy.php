...
class DefaultRenderStrategy implements ListenerAggregateInterface
{
    ...
    public function render(MvcEvent $event)
    {
        ...
        $result->getData()->offsetSet('mvc_event', $event);
        $result->setRequest($request)
               ->setResponse($response);

        if ($event->getParam('layout_output_type') && is_callable(array($renderer, 'setLayoutOutputType'))) {
            $renderer->setLayoutOutputType($event->getParam('layout_output_type'));
        }
    ...
    public function dispatchError(MvcEvent $event)
    {
        ...
        switch ($error) {
            ...
            default:
                ...
                $data['show_exception'] = (isset($_SERVER['RAMPAGE_DEVELOPMENT']) && $_SERVER['RAMPAGE_DEVELOPMENT']);
